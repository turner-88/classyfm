...
| Featured | Tick to make it the large lead card in the Hot Release section on the home page and the `/news` page. Only one Hot Release can be featured at a time — featuring this one unfeatures any other. If no Hot Release is featured, the latest one is shown as the big card automatically. |
...
| Title | Read-only. Fetched automatically from the Spotify link on save (and refreshed whenever you change the Spotify URL or use **Refresh from Spotify**) — there's no field to type into. The URL slug is generated from it automatically. |
...
- **Feature** (star icon): make the item the large lead card at the top of its source's row on the home page and the `/news` page. Only one item per source can be featured at a time — featuring one unfeatures the rest in that source. If a source has nothing featured, its latest item is shown as the big card automatically.
